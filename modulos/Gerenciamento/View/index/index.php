<div class="container">
	<ol class="breadcrumb">
        <li><a href="/">Ínicio</a></li>     
        <li class="active">Gerenciamento</li>
    </ol>	
	<div class="page-header">
		<h1>Gerenciamento <small>Circuito Unifacs </small></h1>
	</div>	
	<p>
		Olá! <strong>Usuário</strong>, Seja bem vindo ao sistema de gerenciamento de inscrições do circuito Unifacs.
	</p>	
	<h2>Escolha uma opção:</h2>
	<div class="row-fluid">
		<div class="col-md-3 col-sm-6">
			<div class="thumbnail">
				<a href="/gerenciamento/eventos/index/usuario/1">
					<img data-src="holder.js/140x140" class="img-thumbnail" alt="140x140" src="data:image/svg+xml;base64," style="width: 140px; height: 140px;">
				</a>
				<div class="caption">
					<h3><a href="/gerenciamento/eventos/index/usuario/1">Eventos</a></h3>
				</div>
			</div>
			
		</div>	
		<div class="col-md-3 col-sm-6">
			<div class="thumbnail">
				<a href="/gerenciamento/locais/index/usuario/1">
					<img data-src="holder.js/140x140" class="img-thumbnail" alt="140x140" src="data:image/svg+xml;base64," style="width: 140px; height: 140px;">
				</a>
				<div class="caption">
					<h3><a href="/gerenciamento/locais/index/usuario/1">Locais</a></h3>
				</div>			
			</div>
			
		</div>	
		<div class="col-md-3 col-sm-6">
			<div class="thumbnail">
				<a href="#">
					<img data-src="holder.js/140x140" class="img-thumbnail" alt="140x140" src="data:image/svg+xml;base64," style="width: 140px; height: 140px;">
				</a>
				<div class="caption">
					<h3><a href="/gerenciamento/inscritos/index/usuario/1">Inscritos</a></h3>
				</div>	
			</div>			
		</div>	
		<div class="col-md-3 col-sm-6">
			<div class="thumbnail">
				<a href="/gerenciamento/usuario/index/usuario/1">
					<img data-src="holder.js/140x140" class="img-thumbnail" alt="140x140" src="data:image/svg+xml;base64," style="width: 140px; height: 140px;">
				</a>
				<div class="caption">
					<h3><a href="/gerenciamento/usuario/index/usuario/1">Usuários</a></h3>
				</div>
			</div>			
		</div>	
	</div>	
</div>